...
class BrandListProvider extends ChangeNotifier {
  ...
  getBrandApiProvider({
    ...
    try {
      ...
      Map<String, dynamic> brandData = await getBrandList(context: context, params: params);

      if (brandData[ApiAndParams.status].toString() == "1") {
        totalData = int.parse(brandData[ApiAndParams.total].toString());
        List<BrandItem> tempBrands = List.from(brandData[ApiAndParams.data]).map((e) => BrandItem.fromJson(e)).toList();
